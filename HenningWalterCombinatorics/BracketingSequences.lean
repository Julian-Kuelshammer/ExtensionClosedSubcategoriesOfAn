...
lemma mem_bracketingFunctions {n : ℕ} (a : Fin n → Fin n) :
a ∈ bracketingFunctions n ↔ (∀ i, i.val ≤ a i) ∧
(∀ i j, i ≤ j → j.val ≤ a i → a j ≤ a i) := by simp [bracketingFunctions, IsBracketing]
...
/-- On the first block of indices, `bracketingNode b c` is given by `b`. -/
@[simp] lemma bracketingNode_val_of_lt {k l : ℕ} (b : Fin k → Fin k) (c : Fin l → Fin l) {v : ℕ}
...
@[simp] lemma bracketingNode_val_mid {k l : ℕ} (b : Fin k → Fin k) (c : Fin l → Fin l) :
    (bracketingNode b c ⟨k, by omega⟩).val = k + l := by
  have he : ⟨k, by omega⟩ = ((0 : Fin 1).natAdd k).castAdd l := Fin.ext (by simp)
  grind [bracketingNode]

/-- On the second block of indices, `bracketingNode b c` is `c` shifted by `k + 1`. -/
@[simp] lemma bracketingNode_val_of_gt {k l : ℕ} (b : Fin k → Fin k) (c : Fin l → Fin l) {t : ℕ}
...
lemma bracketingNode_mem {k l : ℕ} {b : Fin k → Fin k} {c : Fin l → Fin l} (hb : IsBracketing b)
(hc : IsBracketing c) : IsBracketing (bracketingNode b c) := by
  constructor
  · rintro ⟨i, hi⟩
  ...
  · rintro ⟨i, hi⟩ ⟨j, hj⟩ hij hjbc
    ...
    rcases lt_trichotomy i k with h | rfl | h
    · -- both indices lie in the first block, where `bracketingNode b c` is `b`
    ...
    · -- both indices lie in the second block, where `bracketingNode a b` is a shift of `b`
      obtain ⟨s, rfl⟩ : ∃ s, i = k + 1 + s := ⟨i - (k + 1), by omega⟩
      have hs : s < l := by omega
      obtain ⟨t, rfl⟩ : ∃ t, j = k + 1 + t := ⟨j - (k + 1), by omega⟩
      have ht : t < l := by omega
      rw [bracketingNode_val_of_gt b c hs] at hjbc
...
bracketingFunctions k ×ˢ bracketingFunctions (n - k) :=
{ toFun := fun ⟨a, ha⟩ ↦ ⟨⟨Nat.find (isBracketing_exists_final a ((Finset.mem_filter.mp ha).2)),
    ...
    ⟨by simp only [mem_bracketingFunctions];
  ...
  invFun := fun ⟨k, ⟨⟨b, c⟩, hbc⟩⟩ ↦
    ...
    have hb : b ∈ bracketingFunctions k := by
      simpa [bracketingFunctions] using (Finset.mem_product.mp hbc).1
    have hc : c ∈ bracketingFunctions _ := by
      simpa [bracketingFunctions] using (Finset.mem_product.mp hbc).2
    have hj : bracketingNode b c ∈ bracketingFunctions (k.val + 1 + (n - k.val)) := by
      simp only [mem_bracketingFunctions]
      exact bracketingNode_mem ((Finset.mem_filter.mp hb).2) ((Finset.mem_filter.mp hc).2)
    ⟨fun i => Fin.cast h (bracketingNode b c (Fin.cast h.symm i)), by
      simp only [mem_bracketingFunctions]
      have hjB : IsBracketing (bracketingNode b c) := (Finset.mem_filter.mp hj).2
      constructor
      · intro i
      ...
      · intro i j hij hja
        ...
        have hja' :
          ...
          ≤ (bracketingNode b c (Fin.cast h.symm i)).val := by
            exact hja
        rw [Fin.le_def]
  ...
  right_inv := by
    ...
    · apply le_antisymm
      · apply Nat.find_min'
        constructor
        · change bracketingNode b c ⟨k, by omega⟩ = n
          rw [bracketingNode_val_mid]
          omega
        · exact Fin.is_le k
      · refine (Nat.le_find_iff _ _).mpr ?_
        intro m hmk ⟨hmn, hval⟩
        change (bracketingNode b c ⟨m, by omega⟩) = n at hval
        rw [bracketingNode_val_of_lt b c hmk] at hval
        omega
    · apply bracket_pair_heq
      ...
      · intro i j hij
        ...
        simp_rw [hij]
        rw [bracketingNode_val_of_lt b c _]
      · intro i j hij
        ...
        have hK : Nat.find pf2 = (k : ℕ) := by
          apply le_antisymm
          · exact Nat.find_min' pf2 ⟨Nat.lt_succ_iff.mp k.isLt, by rw [bracketingNode_val_mid]; omega⟩
          · refine (Nat.le_find_iff _ _).mpr ?_
            rintro m hmk ⟨hmn, hval⟩
            rw [bracketingNode_val_of_lt b c hmk] at hval
            have := (b ⟨m, hmk⟩).isLt
            omega
        grind [bracketingNode_val_of_gt b c j.isLt]
      }
...
instance {n : ℕ} : SemilatticeInf (bracketingFunctions n) where
  inf := fun a b ↦ ⟨fun i ↦ min (a.1 i) (b.1 i), by grind [mem_bracketingFunctions]⟩
...
instance {n : ℕ} : OrderTop (bracketingFunctions n) where
  top := ⟨fun i => ⟨n - 1, by have := i.isLt; omega⟩, by grind [mem_bracketingFunctions]⟩
...
instance {n : ℕ} : OrderBot (bracketingFunctions n) where
  ...
  bot_le := by intro ⟨a, ha⟩ i; grind [mem_bracketingFunctions]
